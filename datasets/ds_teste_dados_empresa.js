...
function createDataset(fields, constraints, sortFields) {
	...
	var dados_empresa = get_company("22.739.826/0001-00");
	if(dados_empresa != null && dados_empresa.values != null && dados_empresa.rowsCount > 0)
	{
		//sempre o último elemento
		last = (dados_empresa.rowsCount)-1;

		empresa 		 = dados_empresa.getValue(last,"empresa");
		razao_social 	 = dados_empresa.getValue(last,"razao_social");
		cnpj 		 	 = dados_empresa.getValue(last,"cnpj");
		endereco 	 	 = dados_empresa.getValue(last,"endereco");
		complemento 	 = dados_empresa.getValue(last,"complemento");
		bairro 	 	 	 = dados_empresa.getValue(last,"bairro");
		cidade 			 = dados_empresa.getValue(last,"cidade");
		uf_sigla 		 = dados_empresa.getValue(last,"estado_sigla");
		cep 			 = dados_empresa.getValue(last,"cep").replace(".","").replace("-","");
		tel_fixo 	 	 = dados_empresa.getValue(last,"telefone_fixo");
		cliente 		 = dados_empresa.getValue(last,"responsavel");
		
		receiver 		 = dados_empresa.getValue(last,"email_principal");
		cliente_comp     = cliente+" "+dados_empresa.getValue(last,"sobrenome");
    ...
    try{  
        ...
        log.info(tel_fixo);
        log.info(receiver);
        log.info(cliente_comp);
        log.info(cep);

        // atualiza o valor...  
        text = replaceValue(text, "NOMEFANTASIA" , empresa+" T");
        text = replaceValue(text, "NOME" , razao_social+" T");
        text = replaceValue(text, "CGCCFO" , cnpj);
        text = replaceValue(text, "RUA" , endereco);   
        text = replaceValue(text, "NUMERO" , complemento);   
        text = replaceValue(text, "COMPLEMENTO" , complemento);   
        text = replaceValue(text, "BAIRRO" , bairro);   
        text = replaceValue(text, "CIDADE" , cidade);   
        text = replaceValue(text, "CODETD" , uf_sigla); 
        text = replaceValue(text, "CEP" , cep);   
        text = replaceValue(text, "TELEFONE" , tel_fixo);  
        text = replaceValue(text, "EMAIL" , receiver);   
        text = replaceValue(text, "CONTATO" , cliente_comp);
...
/*
 * Função retorna dados da empresa.
 * A consulta é realizada consultando o CNPJ;
 * A consulta é feita no dataset customizado ds_empresas_v3;
*/
function get_company(cnpj)
{
	...
	var dataset = DatasetFactory.getDataset("ds_empresas_v3", null, constraints, null);
